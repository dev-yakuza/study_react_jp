...
`JavaScript`は色んな言語からインスピレーションを受けて作られました。変数の`Scope`や`Closure`は`Lisp`と言う元号で、`Prototype`の継承は`Smalltalk`から派生した`Self`と言うプログラミング言語を参考して作られました。
...
- (余談) es2021: https://gitlab.fixstars.com/quantum/mcpc-quantum/smileboard-opt-app/-/blob/master/gui/.eslintrc#L5
...
- `Ryan Dahl`が`Oracle`に`JavaScript`の商標権の解除をお願いした：https://tinyclouds.org/trademark
